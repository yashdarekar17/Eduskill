                        ...
                        {milestones.map((m: any, i: number) => (
                            <div key={m.id || i} className="flex items-center shrink-0 snap-start">
                                <div
                                    ...
                                    className={`cursor-pointer w-[300px] p-8 rounded-[32px] border-2 transition-all relative ${
                                        selectedMilestone === m.id 
                                        ? 'bg-black border-black text-white shadow-xl' 
                                        : 'bg-[#FBFBFB] border-gray-100 hover:border-black/10'
                                    }`}
                                >
                                    <div className={`absolute -top-3 -right-3 w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold ${
                                        selectedMilestone === m.id ? 'bg-white text-black' : 'bg-black text-white'
                                    }`}>
                                        {i + 1}
                                    </div>
