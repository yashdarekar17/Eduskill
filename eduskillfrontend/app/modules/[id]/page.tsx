    ...
    useEffect(() => {
        const fetchModule = async () => {
            if (isCertificateModule) {
                // Fetch course info if it's the certificate module
                if (courseIdParam) {
                    try {
        ...
        <div>
            ...
            <main className="max-w-[85vw] mx-auto py-12">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500 mb-8">
                    ...
                    <span>/</span>
                    <span className="text-gray-800 font-medium">{moduleData.title}</span>
                </div>

                {/* Module Header */}
                <div className="mb-8">
                    <div className="flex items-center gap-3 mb-2">
                        <span className="px-3 py-1 bg-orange-100 text-[#FF6643] text-xs font-bold rounded-full">
                ...
                {/* Module Content */}
                <div className="bg-white p-8 md:p-12 rounded-[30px] shadow-sm border border-orange-100 mb-8">
                    <h2 className="text-3xl font-extrabold text-gray-800 mb-8 border-l-4 border-[#FF6643] pl-5 leading-tight">
                        Module Content
                    </h2>
                    <div className="module-blog-content">
                ...
                <div className="flex justify-center mb-12">
                    {completed ? (
                        <div className="px-10 py-4 bg-green-100 text-green-700 text-lg font-bold rounded-2xl border border-green-300 flex items-center gap-2">
                            <span className="text-2xl">✓</span> Module Completed!
                        </div>
                    ) : (
                        <button
                            onClick={handleMarkComplete}
                            disabled={marking}
                            className="px-10 py-4 bg-[#FF6643] text-white text-lg font-bold rounded-2xl hover:bg-[#e65c00] transition-all shadow-lg hover:-translate-y-1 disabled:opacity-60 disabled:cursor-not-allowed"
                        >
                ...
                <div className="flex justify-center mt-10">
                    <Link href={`/viewdetails/${moduleData.course_id}`}>
                        <button className="px-8 py-3 border-2 border-[#FF6643] text-[#FF6643] font-bold rounded-xl hover:bg-[#FF6643] hover:text-white transition-all">
                            ← Back to Course
                        </button>
                    </Link>
                </div>
